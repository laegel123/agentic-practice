...
> 전체 시스템 맥락은 모노레포 문서 [`../docs/`](../docs/) 를, 이 모듈의 상세는 [`docs/`](./docs/) 를 본다.
...
.\gradlew.bat :batch:test       # 테스트 — 현재 테스트 소스 없음
```
...
│   ├── SettlementJob            정산: 전체 주문 매출 합계
│   ├── DailySalesAggregationJob 일일 집계: 오늘 주문 건수/매출
...
| 순서 | 잡 | 동작 | 읽는 테이블 |
|------|-----|------|-----------|
| 1 | `SettlementJob.settle()` | 전체 주문 `totalAmount` 합산 → 총 매출 출력 | `orders` |
| 2 | `DailySalesAggregationJob.aggregate()` | 오늘(`LocalDate.now()`) 주문 건수·매출 집계 | `orders` |
...
- **엔티티는 `*Row` 프로젝션 패턴**이다. 공통 컨벤션과 달리 `BaseTimeEntity` 미상속, `@GeneratedValue` 없음(`@Id` 만),
  **getter 만**(setter 없음), 필요한 컬럼만 매핑한다. 새 읽기 모델도 이 패턴을 따른다 — 상세는
...
- ⚠️ 아래는 known-issues 에 등록된 문제다. **새 코드에서 모방하지 말고**, 손대는 김에 개선한다.
  ...
  - **취소주문 매출 포함**: `SettlementJob`·`DailySalesAggregationJob` 이 `status` 를 매핑만 하고 필터에 안 써
    `CANCELLED` 주문도 매출에 합산한다(batch 신규발견). [`docs/known-issues.md`](./docs/known-issues.md).
  - **`OrderStatus` enum 복제**: ecommerce 와 별개로 정의 → 드리프트 위험. [`docs/known-issues.md`](./docs/known-issues.md).
