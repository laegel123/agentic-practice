...
.\gradlew.bat test                         # 전체 테스트 (28개, 현재 동작 고정 characterization)
...
- **검증 루프 구축 완료**: `common-util`/`ecommerce-service`/`payment-service`에 JUnit5 + Mockito +
  AssertJ 기반 characterization 테스트 28개(`.\gradlew.bat test`, 전부 green). 현재 동작(버그 포함)을
  고정해 향후 리팩토링의 안전망 역할을 한다. 테스트는 인메모리 H2 프로파일(`test`)로 실 DB와 격리됨.
