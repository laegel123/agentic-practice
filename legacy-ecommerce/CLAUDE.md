...
.\gradlew.bat test                         # 전체 테스트 (31개: 동작 고정 characterization + E1 보안 회귀)
...
- `InventoryService.reserve()` == `confirm()` → 주문마다 재고 **이중 차감**.
...
- `RefundService` 과다 환불 미검증(`REFUND_EXCEEDS_PAYMENT` 미사용).
...
- ⚠️ **보안 우선 남은 2건**: `POST /admin/refunds` 무인증(A1), `CryptoUtils` MD5·무 salt 비밀번호 해시(CU1). (`ProductSearchDao` SQL 인젝션 E1 은 ✅ 2026-06-15 파라미터 바인딩으로 수정됨 — 모방 금지 대상 아님.)

전체 목록·우선순위: [`docs/known-issues.md`](./docs/known-issues.md). 결정 배경: [`docs/adr/`](./docs/adr/).

## 현재 상태

- **검증 루프 구축 완료**: `common-util`/`ecommerce-service`/`payment-service`에 JUnit5 + Mockito +
  AssertJ 기반 테스트 31개(`.\gradlew.bat test`, 전부 green) — 현재 동작(버그 포함)을 고정하는
  characterization 28개 + E1 SQL 인젝션 회귀 테스트 3개(`ProductSearchDaoTest`). 향후 리팩토링의
  안전망 역할을 한다. 테스트는 인메모리 H2 프로파일(`test`)로 실 DB와 격리됨.
- **보안 수정 진행 중**: E1(SQL 인젝션) ✅ 완료(2026-06-15) — `ProductSearchDao` 파라미터 바인딩 +
  회귀 테스트 `ProductSearchDaoTest`. 남은 보안 최우선: A1(무인증 환불)·CU1(MD5 해시).
- 다음 과제: 위 안전망 위에서 [`docs/known-issues.md`](./docs/known-issues.md)의 버그 수정/리팩토링.
