...
.\gradlew.bat test                         # 전체 테스트 (34개: 동작 고정 characterization + E1·A1 보안 회귀)
...
- ⚠️ **보안 우선 남은 1건**: `CryptoUtils` MD5·무 salt 비밀번호 해시(CU1). (이미 수정되어 **모방 금지 대상 아님**: `ProductSearchDao` SQL 인젝션 E1·`POST /admin/refunds` 무인증 A1 — 둘 다 ✅ 2026-06-15 수정.)
...
- **검증 루프 구축 완료**: `common-util`/`ecommerce-service`/`payment-service`/`admin`에 JUnit5 + Mockito +
  AssertJ 기반 테스트 34개(`.\gradlew.bat test`, 전부 green) — 현재 동작(버그 포함)을 고정하는
  characterization 28개 + 보안 회귀 6개(E1 `ProductSearchDaoTest` 3 + A1 `AdminRefundControllerTest` 3).
  향후 리팩토링의 안전망 역할을 한다. 테스트는 인메모리 H2 프로파일(`test`)로 실 DB와 격리됨.
- **보안 수정 진행 중**: E1(SQL 인젝션)·A1(무인증 환불) ✅ 완료(2026-06-15) — E1 `ProductSearchDao` 파라미터
  바인딩, A1 `AdminRefundController` 에 `X-Admin-Token` 검사 추가(각각 회귀 테스트 동반). 남은 보안 최우선: CU1(MD5 해시).
