...
.\gradlew.bat test                         # 전체 테스트 (65개: characterization + 버그수정 회귀 B1·B2·B3·B4·B5·B6·B7·BT1 + E1·A1·CU1 보안 회귀 + 동작보존 정리 회귀 R4·R6·CU2)
...
- 서비스 간 HTTP가 raw `Map`(타입 없음, R2). 서비스 URL/`admin.token` 하드코딩(R5). (`RestTemplate` 타임아웃 미설정 R8 은 ✅ 수정 — 아래 참조.)
...
- **검증 루프 구축 완료**: `common-util`/`core-framework`/`ecommerce-service`/`payment-service`/`admin`/`batch`에 JUnit5 + Mockito +
  AssertJ 기반 테스트 65개(`.\gradlew.bat test`, 전부 green) — characterization 28개 + 버그수정 회귀 17개
...
- 다음 과제: **정합성·고영향 버그·동작보존 정리 전부 완료**. 다음은 **구조 리팩토링**: R1(God method 추출)·R2(Map→DTO)·BT2(enum 동기화). [`docs/known-issues.md`](./docs/known-issues.md) "권장 처리 순서" 5단계 참고.
