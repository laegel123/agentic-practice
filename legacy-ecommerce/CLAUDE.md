...
.\gradlew.bat test                         # 전체 테스트 (77개: characterization + 버그수정 회귀 B1·B2·B3·B4·B5·B6·B7·BT1 + E1·A1·CU1 보안 회귀 + 동작보존 정리 회귀 R4·R6·CU2 + 구조 리팩토링 회귀 R2 PaymentClientTest + 리뷰 후속 보강 10개[B5 page≤0 클램프·AdminAuth fail-closed·B6 음수환불·hashPassword null])
...
- 서비스 URL 하드코딩(R5 잔여 — URL·DB 경로). (`admin.token` 의 **fail-open 기본값**은 ✅ 제거 — 환경변수 `ADMIN_TOKEN` 외부 주입·미설정 시 기동 실패로 바꿈, 아래 리뷰 후속 참조. 서비스 간 HTTP raw `Map` R2·`RestTemplate` 타임아웃 미설정 R8 도 ✅ 수정 — 아래 참조.)
...
- ✅ **리뷰 후속 보강 4건 수정 완료(2026-06-16)**: ① **B5 후속** — `PageRequestDto.getOffset()` 이 `page≤0`(어드민 기본 호출 `?page=0` 포함)에서 음수 offset → `subList` 500 크래시를 새로 만들었던 것을 `Math.max(1, page)` 클램프로 차단(+ `AdminProductController` 페이지 기본값 `0→1`). ② **A1 후속** — `AdminAuth` 의 fail-open 기본 시크릿(`@Value("${admin.token:admin-secret}")`) 제거 → 빈 토큰이면 기동 실패(fail-closed)·`MessageDigest.isEqual` 상수시간 비교. ③ **B6 후속** — `RefundService` 가 음수/0 환불액을 `INVALID_REFUND_AMOUNT`(PM004)로 거부(음수는 누계를 줄여 과다환불 가드를 우회). ④ `CryptoUtils.hashPassword(null)` NPE → `IllegalArgumentException`. 회귀 10개 동반(`PageRequestDtoTest`·`ProductServiceTest` 각 +2, `AdminAuthTest` 신규 3, `RefundServiceTest` +2, `CryptoUtilsTest` +1). ⚠️ **남은 한계**(이번에 안 고침): refund/reserve 의 check-then-act **TOCTOU 경합**(단일 H2라 당장 영향 작음), 금액 `double` 누적오차(BigDecimal 전환은 대형 과제).
...
- **검증 루프 구축 완료**: `common-util`/`core-framework`/`ecommerce-service`/`payment-service`/`admin`/`batch`에 JUnit5 + Mockito +
  AssertJ 기반 테스트 77개(`.\gradlew.bat test`, 전부 green) — characterization 28개 + 버그수정 회귀 17개
  ...
  리뷰 후속 보강 10개(B5 page≤0 클램프 `PageRequestDtoTest`+2·`ProductServiceTest`+2 + AdminAuth fail-closed `AdminAuthTest` 신규 3 + B6 음수환불 `RefundServiceTest`+2 + hashPassword null `CryptoUtilsTest`+1).
