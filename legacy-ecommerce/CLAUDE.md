...
.\gradlew.bat test                         # 전체 테스트 (40개: 동작 고정 characterization + E1·A1·CU1 보안 회귀)
...
- **검증 루프 구축 완료**: `common-util`/`ecommerce-service`/`payment-service`/`admin`에 JUnit5 + Mockito +
  AssertJ 기반 테스트 40개(`.\gradlew.bat test`, 전부 green) — 현재 동작(버그 포함)을 고정하는
  characterization 28개 + 보안 회귀 12개(E1 `ProductSearchDaoTest` 3 + A1 `AdminRefundControllerTest` 3 + CU1 `CryptoUtilsTest` 6).
...
- 다음 과제: 위 안전망 위에서 [`docs/known-issues.md`](./docs/known-issues.md)의 고영향 버그 수정/리팩토링(B1·B2·B3·B4·B6·BT1).
