...
- **공유 H2 파일 DB 분리/전환**: ecommerce↔batch 공유 구조의 결합·동시성 한계. [ADR-0002](./adr/0002-shared-h2-file-database.md). ← **다음 대형 과제**(BigDecimal 컬럼도 공유 DB 라 두 모듈을 함께 옮겨야 함, ADR-0006 참고).
...
6. **대형 과제**: ~~BigDecimal 전환~~ ✅ **완료(2026-06-16, [ADR-0006])** — 금액 전사 `BigDecimal`(scale 2/HALF_UP) 전환, B3 근본 해결. ~~설정 외부화(R5 — URL·DB 경로·자격증명)~~ ✅ **완료(2026-06-16, [ADR-0007])** — 전 모듈 `${ENV:기본값}` 외부화(동작 보존) + 4모듈 `ConfigExternalizationTest`. 남은 대형 과제: **DB 구조 분리**([ADR-0002]). (B7 타임존은 ✅ 3단계에서 완료.)
