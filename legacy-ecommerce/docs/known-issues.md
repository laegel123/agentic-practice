...
| R5 | 설정 전반 (`application.yml`, `@Value` 기본값) | 서비스 URL·DB 경로가 **하드코딩**(운영 분리 불가) — 미해결. **`admin.token` fail-open 부분은 ✅ 수정(2026-06-16, 리뷰 차단):** `@Value("${admin.token:admin-secret}")` 의 공개 기본 시크릿을 제거 → `admin.token` 미설정/빈 값이면 **`AdminAuth` 생성 시 `IllegalStateException` 으로 기동 실패**(fail-closed), `application.yml` 은 `${ADMIN_TOKEN:}` 로 환경변수 주입. 토큰 비교도 `equals` → **`MessageDigest.isEqual`(상수시간)**. 회귀 `AdminAuthTest`. (URL·DB 경로 외부화는 R5 대형 과제로 잔존.) | 동작보존(외부화) — 부분 완료 |
...
6. **대형 과제**: ~~BigDecimal 전환~~ ✅ **완료(2026-06-16, [ADR-0006])** — 금액 전사 `BigDecimal`(scale 2/HALF_UP) 전환, B3 근본 해결. 남은 대형 과제: **DB 구조 분리**([ADR-0002]), **설정 외부화(R5 — URL·DB 경로; `admin.token` 은 ✅ 완료)**. (B7 타임존은 ✅ 3단계에서 완료.)
