...
`common-util` 모듈에 **한정된** 아키텍처·관례·이슈를 정리한다. 모노레포 전체에 공통으로 적용되는
...
| 문서 | 내용 |
|------|------|
| [`architecture.md`](./architecture.md) | 책임, 클래스 인벤토리, 상태/스레드 안전성, 의존성·빌드 |
| [`code-conventions.md`](./code-conventions.md) | 정적 유틸 클래스 패턴 등 모듈 고유 관례와 공통 컨벤션 대비 차이점 |
| [`known-issues.md`](./known-issues.md) | common-util 한정 결함·기술부채 메모 (MD5 해시, JSON 오류처리 등) |
